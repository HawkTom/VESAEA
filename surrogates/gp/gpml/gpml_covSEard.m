function [A, B] = gpml_covSEard(loghyper, x, z)
...
if nargin == 2
    K = sf2*exp(-gpml_sq_dist(diag(1./ell)*x')/2);
    A = K;
elseif nargout == 2                              % compute test set covariances
    A = sf2*ones(size(z,1),1);
    B = sf2*exp(-gpml_sq_dist(diag(1./ell)*x',diag(1./ell)*z')/2);
