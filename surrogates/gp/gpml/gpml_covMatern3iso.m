function [A, B] = gpml_covMatern3iso(loghyper, x, z)
...
if nargin == 2                                      % compute covariance matrix
    A = sqrt(gpml_sq_dist(x'));
    K = sf2*exp(-A).*(1+A);
    A = K;
elseif nargout == 2                              % compute test set covariances
    z = sqrt(3)*z/ell;
    A = sf2;
    B = sqrt(gpml_sq_dist(x',z'));
    B = sf2*exp(-B).*(1+B);
else                                              % compute derivative matrices
    if z == 1
        A = sf2*gpml_sq_dist(x').*exp(-sqrt(gpml_sq_dist(x')));
    else
        % check for correct dimension of the previously calculated kernel matrix
        if any(size(K)~=n)
            K = sqrt(gpml_sq_dist(x'));
            K = sf2*exp(-K).*(1+K);
        end
        A = 2*K;
        clear K;
