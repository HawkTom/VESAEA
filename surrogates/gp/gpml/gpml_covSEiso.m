function [A, B] = gpml_covSEiso(loghyper, x, z)
...
if nargin == 2
    A = sf2*exp(-gpml_sq_dist(x'/ell)/2);
elseif nargout == 2                              % compute test set covariances
    A = sf2*ones(size(z,1),1);
    B = sf2*exp(-gpml_sq_dist(x'/ell,z'/ell)/2);
else                                                % compute derivative matrix
    if z == 1                                                   % first parameter
        A = sf2*exp(-gpml_sq_dist(x'/ell)/2).*gpml_sq_dist(x'/ell);
    else                                                       % second parameter
        A = 2*sf2*exp(-gpml_sq_dist(x'/ell)/2);
    end
end
