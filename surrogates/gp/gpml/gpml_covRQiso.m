function [A, B] = gpml_covRQiso(loghyper, x, z)
...
if nargin == 2                                      % compute covariance matrix
    K = (1+0.5*gpml_sq_dist(x'/ell)/alpha);
    A = sf2*(K.^(-alpha));
elseif nargout == 2                              % compute test set covariances
    A = sf2*ones(size(z,1),1);
    B = sf2*((1+0.5*gpml_sq_dist(x'/ell,z'/ell)/alpha).^(-alpha));
    ...
    if z == 1                                           % length scale parameters
        A = sf2*K.^(-alpha-1).*gpml_sq_dist(x'/ell);
    elseif z == 2                                           % magnitude parameter
        A = 2*sf2*(K.^(-alpha));
    else
        A = sf2*K.^(-alpha).*(0.5*gpml_sq_dist(x'/ell)./K - alpha*log(K));
        clear K;
    end
end
