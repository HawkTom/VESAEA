function [A, B] = gpml_covRQard(logtheta, x, z)
...
if nargin == 2
    K = (1+0.5*gpml_sq_dist(diag(1./ell)*x')/alpha);
    A = sf2*(K.^(-alpha));
elseif nargout == 2                              % compute test set covariances
    A = sf2*ones(size(z,1),1);
    B = sf2*((1+0.5*gpml_sq_dist(diag(1./ell)*x',diag(1./ell)*z')/alpha).^(-alpha));
    ...
    if z <= D                                           % length scale parameters
        A = sf2*K.^(-alpha-1).*gpml_sq_dist(x(:,z)'/ell(z));
    elseif z == D+1                                         % magnitude parameter
        A = 2*sf2*(K.^(-alpha));
    else
        A = sf2*K.^(-alpha).*(0.5*gpml_sq_dist(diag(1./ell)*x')./K - alpha*log(K));
        clear K;
    end
end
